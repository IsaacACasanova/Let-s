...
- (PFQuery *)queryForTable {
    ...
    [query whereKey:@"Follower" matchesKey:@"objectId" inQuery:query2];
    PFQuery *userQuery = [PFUser query];
    [userQuery whereKey:@"objectId" matchesKey:@"Following" inQuery:query];
...
/*-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if([[segue identifier] isEqualToString:@"ShowAFriend"]){
        ...
        PFQuery *queryforTheUser = [PFUser query];
        [queryforTheUser whereKey:@"objectId" equalTo:userId];
        PFUser *theUser = queryforTheUser.getFirstObject;
        [vc grabUserInfo:theUser];
